    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[850] bg-black/95 backdrop-blur-xl md:hidden"
          onClick={onClose}
          role="dialog"
          aria-modal="true"
          aria-label="모바일 메뉴"
            ...
            {navItems.map((item, index) => (
              <motion.div key={item.href} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                <Link
                  ...
                  className={`relative inline-block p-4 text-4xl font-black italic leading-relaxed tracking-tight transition-colors ${
