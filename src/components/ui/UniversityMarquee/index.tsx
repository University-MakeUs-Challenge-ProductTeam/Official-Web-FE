    ...
    <section className="relative overflow-hidden bg-black py-40">
      ...
      <div className="container mx-auto mb-20 px-6 text-center">
        <h2 className="text-3xl font-black italic tracking-tighter text-white sm:text-5xl">
          JOINED BY <span className="text-[#52E560]">TOP UNIVERSITIES</span>
        </h2>
            ...
            <div key={`${univ.participateSchoolId}-${index}`} className="group/item flex items-center gap-4 grayscale transition-all hover:grayscale-0">
              <div className="relative flex size-16 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-white/5 p-3 transition-colors group-hover/item:border-[#52E560]/50">
                {univ.logoImageUrl ? (
                  ...
                  <span className="text-[10px] font-black italic text-white/50 group-hover/item:text-[#52E560]">{univ.schoolName.substring(0, 1)}</span>
                )}
              </div>
