...
function ActivityBox() {
  ...
  return (
    <div className="flex flex-col py-20">
      ...
      >
        <span className="text-xs font-bold uppercase tracking-widest text-[#52E560]">Details</span>
        ...
        {!isLoading && activities
          ? ACTIVITY_CONTENT.map((item, idx) => (
              <motion.div
                ...
                viewport={{ once: true }}
                style={{ willChange: 'transform, opacity' }}
                className="group relative flex transform-gpu flex-col gap-4 overflow-hidden rounded-3xl border border-white/5 bg-white/5 p-8 backdrop-blur-3xl transition-all hover:border-[#52E560]/30 hover:bg-white/[0.08]"
              >
                <div className="absolute right-0 top-0 size-10 opacity-10 transition-opacity group-hover:opacity-100">
                  <div className="absolute right-4 top-4 h-[2px] w-4 rotate-45 bg-[#52E560]" />
                  <div className="absolute right-4 top-4 h-4 w-[2px] rotate-45 bg-[#52E560]" />
                </div>

                <Typography className="text-xs font-black uppercase tracking-widest text-[#52E560]">{item.label}</Typography>
