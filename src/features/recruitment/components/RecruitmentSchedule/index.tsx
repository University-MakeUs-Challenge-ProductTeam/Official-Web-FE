    ...
    <div className="group relative flex flex-1 flex-col gap-8 rounded-[40px] border border-white/5 bg-white/[0.02] p-6 backdrop-blur-3xl transition-all hover:border-[#52E560]/20 md:p-10">
          ...
          return (
            <div className="group/item relative flex flex-col gap-2" key={label}>
              <div className="flex items-center gap-3">
                <div className="size-1.5 rounded-full bg-[#52E560] opacity-50 shadow-[0_0_10px_#52E560] transition-opacity group-hover/item:opacity-100" />
                <Typography className="text-[10px] font-black uppercase tracking-[0.2em] text-[#52E560]/60">{label}</Typography>
              </div>
              ...
              {label !== '최종 합격자 발표' && <div className="absolute left-[3px] top-6 h-10 w-px bg-gradient-to-b from-[#52E560]/20 to-transparent" />}
            </div>
          );
        })}
      </div>
      ...
      <div className="mt-auto border-t border-white/5 pt-6">
        <Typography className="text-[10px] font-medium leading-relaxed text-white/20">
          * 추가모집, 일정변경 등 자세한 사항은 <span className="text-[#52E560]/40">@학교 인스타그램</span>을 참고해주세요.
        </Typography>
      </div>
    </div>
  );
